...
def layers(vgg_layer3_out, vgg_layer4_out, vgg_layer7_out, num_classes):
    ...
    output_4 = tf.layers.conv2d(vgg_layer4_out, num_classes, 1, padding='same', kernel_regularizer=tf.contrib.layers.l2_regularizer(1e-3))
    # Skip connection by adding pooling layer output
    skip1 = tf.add(output, output_4)
    
    output = tf.layers.conv2d_transpose(skip1, num_classes, 4, 2, padding='same', kernel_regularizer=tf.contrib.layers.l2_regularizer(1e-3))
    output_3 = tf.layers.conv2d(vgg_layer3_out, num_classes, 1, padding='same', kernel_regularizer=tf.contrib.layers.l2_regularizer(1e-3))
    skip2 = tf.add(output, output_3)
...
def train_nn(sess, epochs, batch_size, get_batches_fn, train_op, cross_entropy_loss, input_image,
    ...
    for i in range(epochs):
        ...
        for image, label in get_batches_fn(batch_size):
            training_op,loss = sess.run([train_op,cross_entropy_loss], feed_dict={input_image:image, correct_label:label, keep_prob:0.5, learning_rate:rate})
        print("Epoch number: %d, learning_rate: %.5f, loss: %.4f"%(i+1,rate,loss))
...
def run():
    ...
    with tf.Session() as sess:
        ...
        print("Total runtime of training: %d"%diff)
